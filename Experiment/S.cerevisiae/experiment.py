...
def transfer_fasta(fasta):
    ...
    protein_array_row=['']
    for row in fasta:
        if (len(row)>0 and row[0].find('DIP-')>=0):
            if (len(dip_number_row)>0 and len(protein_array_row[0])>0):
                fasta_hash[dip_number_row[0]]=protein_array_row[0]
                protein_array_row=['']
            dip_number_row=row
        elif (len(row)>0 and row[0].find('DIP-')<0):
            protein_array_row[0]+=row[0]
    #print(fasta_hash)
    return fasta_hash       
...
def construct_array(dip_matrix,locate_feature,locate_fasta,label):
    ...
    for row in dip_matrix:
        if (len(row)<=0 ):
            continue
        if (row[0].find('N')<0 or len(row[0])<=0 ):
            continue
        row=row[0].split('-')
        dip_proteinA,dip_proteinB=row[0],row[1]
        short_itemA=dip_proteinA[0:dip_proteinA.find('N')]
        short_itemB=dip_proteinB[0:dip_proteinB.find('N')]
    ...
    for temp_interact_row in temp_interact_hash:
        ...
        if (len(dip_proteinA)<80 or len(dip_proteinB)<80):
            continue      
...
def search_match(protein_number,locate_fasta):
    ...
    for protein_array in locate_fasta:  
        if (protein_array.find('DIP-'+protein_number+'N')>=0):
